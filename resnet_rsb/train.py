...
# AugSub
parser.add_argument('--augsub', default='none', choices=['none', 'masking'], type=str, help='Types for AugSub')
parser.add_argument('--augsub-ratio', default=0.5, type=float, help='Drop probability for AugSub')
...
def main():
    ...
    model = create_model(args.model, pretrained=args.pretrained, num_classes=args.num_classes, drop_rate=args.drop,
        drop_connect_rate=args.drop_connect,  # DEPRECATED, use drop_path
        drop_path_rate=args.drop_path, drop_block_rate=args.drop_block, global_pool=args.gp,
        bn_momentum=args.bn_momentum, bn_eps=args.bn_eps, scriptable=args.torchscript,
    ...
    if use_amp == 'native':
        ...
        raise NotImplementedError(f'Only Native Torch AMP is supported.')

    # optionally resume from a checkpoint
    resume_epoch = None
    if args.resume:
        resume_epoch = resume_checkpoint(model, args.resume, optimizer=None if args.no_resume_opt else optimizer,
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank],
                              broadcast_buffers=not args.no_ddp_bb)  # NOTE: EMA model does not need to be wrapped by DDP
    ...
    dataset_train = create_dataset(args.dataset, root=args.data_dir, split=args.train_split, is_training=True,
        class_map=args.class_map, download=args.dataset_download, batch_size=args.batch_size,
    ...
    loader_train = create_loader(dataset_train, input_size=data_config['input_size'], batch_size=args.batch_size,
        is_training=True, use_prefetcher=args.prefetcher, no_aug=args.no_aug, re_prob=args.reprob, re_mode=args.remode,
        re_count=args.recount, re_split=args.resplit, scale=args.scale, ratio=args.ratio, hflip=args.hflip,
        vflip=args.vflip, color_jitter=args.color_jitter, auto_augment=args.aa, num_aug_repeats=args.aug_repeats,
        num_aug_splits=num_aug_splits, interpolation=train_interpolation, mean=data_config['mean'],
        std=data_config['std'], num_workers=args.workers, distributed=args.distributed, collate_fn=collate_fn,
    ...
    loader_eval = create_loader(dataset_eval, input_size=data_config['input_size'],
        batch_size=args.validation_batch_size or args.batch_size, is_training=False, use_prefetcher=args.prefetcher,
    ...
    if args.rank == 0:
        ...
        saver = CheckpointSaver(model=model, optimizer=optimizer, args=args, model_ema=model_ema,
            amp_scaler=loss_scaler, checkpoint_dir=output_dir, recovery_dir=output_dir, decreasing=decreasing,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(epoch, model, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir, amp_autocast=amp_autocast,
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                ema_eval_metrics = validate(model_ema.module, loader_eval, validate_loss_fn, args,
            ...
            log_stats = {**{'pretrain/' + f'train/{k}': v for k, v in train_metrics.items()},
                         **{'pretrain/' + f'test/{k}': v for k, v in eval_metrics.items()}, 'epoch': epoch}
...
def train_one_epoch(epoch, model, loader, optimizer, loss_fn, args, lr_scheduler=None, saver=None, output_dir=None,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            optimizer.zero_grad()
            if args.augsub != 'none':
                loss_scaler(loss/2, optimizer, clip_grad=args.clip_grad,
                            parameters=model_parameters(model), create_graph=second_order, update_grad=False)

                # Sub-model forward
                output_sub = model(input, augsub_type=args.augsub, augsub_ratio=args.augsub_ratio)
                target_sub = F.sigmoid(output.detach()) if args.bce_loss else F.softmax(output.detach(), dim=-1)
                loss_sub = loss_fn(output_sub, target_sub)
                loss_scaler(loss_sub/2, optimizer, clip_grad=args.clip_grad,
                            parameters=model_parameters(model), create_graph=second_order, update_grad=True)
            else:
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                _logger.info('Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                             ...
                             'LR: {lr:.3e}  '
                             'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(epoch, batch_idx, len(loader),
                    100. * batch_idx / last_idx, loss=losses_m, batch_time=batch_time_m,
                    rate=input.size(0) * args.world_size / batch_time_m.val,
                    rate_avg=input.size(0) * args.world_size / batch_time_m.avg, lr=lr, data_time=data_time_m))

                if args.save_images and output_dir:
                    torchvision.utils.save_image(input, os.path.join(output_dir, 'train-batch-%d.jpg' % batch_idx),
