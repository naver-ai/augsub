...
class augsub_vit_models(vit_models):

    def __init__(self, **kwargs):
        super(augsub_vit_models, self).__init__(**kwargs)
    ...
    def forward_features_dropout(self, x, dropout_ratio=0.0):
        for depth, block in enumerate(self.blocks):
            block.attn.proj_drop.p = dropout_ratio
            block.mlp.drop.p = dropout_ratio
        x = self.forward_features(x)
        for depth, block in enumerate(self.blocks):
    ...
    def forward_features_droppath(self, x, droppath_ratio=0.0):
        dprs = []
        for depth, block in enumerate(self.blocks):
            dprs.append(block.drop_path.drop_prob)
            block.drop_path.drop_prob = droppath_ratio
        x = self.forward_features(x)
        for block, dpr in zip(self.blocks, dprs):
            block.drop_path.drop_prob = dpr

        return x

    def forward(self, x, augsub_type='none', augsub_ratio=0.0):
        if augsub_type == 'dropout':
            x = self.forward_features_dropout(x, augsub_ratio)
        elif augsub_type == 'droppath':
            x = self.forward_features_droppath(x, augsub_ratio)
